...
def plot_labelled_scatter(X, y, class_labels, title=None, xlabel=None, ylabel=None):
...
def plot_class_regions_for_classifier_subplot(
    ...
    y_min = X[:, 1].min()
    y_max = X[:, 1].max()
    x2, y2 = numpy.meshgrid(
        numpy.arange(x_min - k, x_max + k, h), numpy.arange(y_min - k, y_max + k, h)
    )
    ...
    subplot.scatter(
        X[:, 0], X[:, 1], c=y, cmap=cmap_bold, s=plot_symbol_size, edgecolor="black"
    )
...
def plot_fruit_knn(X, y, n_neighbors, weights):
    ...
    xx, yy = numpy.meshgrid(
        numpy.arange(x_min, x_max, mesh_step_size),
